...
export function getSharedArgs(options: StillOptions | StreamOptions): string[] {
  return [
    ...
    ...(options.exposureCompensation
      ? ['--ev', options.exposureCompensation.toString()]
      : []),
    ...
    ...(options.exposureMode
      ? ['--exposure', options.exposureMode.toString()]
      : []),
     ...
     */
    ...(options.analogGain
      ? ['--analoggain', options.analogGain.toString()]
      : []),
     ...
     */
    ...(options.digitalGain
      ? ['--digitalgain', options.digitalGain.toString()]
      : []),
  ];
}
